...
print("nodes_set:",nodes_set(adj_time))
print("="*20)
...
print("BFS:(time,path):",BFS_Single(adj_time,'6','6'))
print("="*20)
...
print("BFS:{tgt:[arrive_time,latency]}",Arrive_time_and_Latency(adj_time,'6'))
print("="*20)
...
#Global reachability and average latency of the original graph
global_reachable,avg_latency = sensitive(adj_time)
print(f"global reachable:{global_reachable} || avg latency:{avg_latency}")
print("="*20)
...
#test
Edge = 'e10'
rm_edge = modify(adj_time,'2','7',None,Edge)
print(rm_edge)
if rm_edge:
    print(f"Remove edge:{Edge}")
    print(f"Before rm arrive time is {rm_edge[0][0]},before rm path is {rm_edge[0][1]}")
    print(f"After rm arrive time is {rm_edge[1][0]},after rm path is {rm_edge[1][1]}")
    print(f"Before rm global reachable rate is {rm_edge[2][0]},before avg latency is {rm_edge[2][1]}")
    print(f"After rm global reachable rate is {rm_edge[3][0]},after rm avg latency is {rm_edge[3][1]}")
    print("="*20)


Vertex = '7'
rm_vertex = modify(adj_time,'2','7',Vertex)

if rm_vertex:
    print(f"Remove vertex:{Vertex}")
    print(f"Before rm arrive time is {rm_vertex[0][0]},before rm path is {rm_vertex[0][1]}")
    print(f"After rm arrive time is {rm_vertex[1][0]},after rm path is {rm_vertex[1][1]}")
    print(f"Before rm global reachable rate is {rm_vertex[2][0]},before avg latency is {rm_vertex[2][1]}")
    print(f"After rm global reachable rate is {rm_vertex[3][0]},after rm avg latency is {rm_vertex[3][1]}")
    print("="*20)


# Get user input for the edge(s) to remove
Edges = input("Enter the edge(s) you want to remove (space-separated if multiple): ").strip()

# Call the modify function
rm_edges = modify(adj_time, '2', '7', Vertexes=None, Edges=Edges)

# Print results if modification was successful
if rm_edges:
    print(f"Remove edge(s): {Edges}")
    print(f"Before rm arrive time is {rm_edges[0][0]}, before rm path is {rm_edges[0][1]}")
    print(f"After rm arrive time is {rm_edges[1][0]}, after rm path is {rm_edges[1][1]}")
    print(f"Before rm global reachable rate is {rm_edges[2][0]}, before avg latency is {rm_edges[2][1]}")
    print(f"After rm global reachable rate is {rm_edges[3][0]}, after avg latency is {rm_edges[3][1]}")
    print("="*20)


# Get user input for the vertex to remove
Vertex = input("Enter the vertex you want to remove: ").strip()

# Call the modify function
rm_vertexes = modify(adj_time, '2', '7', Vertex)

# Print results if modification was successful
if rm_vertexes:
    print(f"Remove vertexes: {Vertex}")
    print(f"Before rm arrive time is {rm_vertexes[0][0]}, before rm path is {rm_vertexes[0][1]}")
    print(f"After rm arrive time is {rm_vertexes[1][0]}, after rm path is {rm_vertexes[1][1]}")
    print(f"Before rm global reachable rate is {rm_vertexes[2][0]}, before avg latency is {rm_vertexes[2][1]}")
    print(f"After rm global reachable rate is {rm_vertexes[3][0]}, after rm avg latency is {rm_vertexes[3][1]}")
